...
class BuildDateCheck(AbstractCheck.AbstractFilesCheck):
    def __init__(self):
        ...
        self.looksliketime = re.compile('\b(2[0-3]|[01]?[0-9]):([0-5]?[0-9]):([0-5]?[0-9])\b')
        self.istoday = re.compile('\b' + time.strftime("%b %d %Y") + '\b')
    ...
    def check_file(self, pkg, filename):
        ...
        if len(grep_date):
            printWarning(pkg, "file-contains-current-date", filename, grep_date)

        grep_time = pkg.grep(self.looksliketime, filename)

        if len(grep_date) and len(grep_time):
            printError(pkg, "file-contains-build-and-time", filename, grep_time)
...
'file-contains-build-and-time',
"""Your file uses C __DATE and __TIME__ this causes the package to rebuild when not needed"""
)
